...
export class TaskScope extends Scope<Task> {
	...
	excludeDraftsOfOthers(creatorId: EntityId): TaskScope {
		this.addQuery({
			$or: [this.getByDraftForCreatorQuery(true, creatorId), this.getByDraftQuery(false)],
		});

		return this;
	}

	byDraftForCreator(isDraft: boolean, creatorId: EntityId): TaskScope {
		this.addQuery(this.getByDraftForCreatorQuery(true, creatorId));
	...
	private getByDraftForCreatorQuery(isDraft: boolean, creatorId: EntityId): FilterQuery<Task> {
